...
async function movieDetalhe(movies, type = "tv"){
    try{   
...
async function movieImages(movies, type = "tv"){
    try{   
...
async function iniciandoAPICreditos(movies, type = "tv"){
    try{
...
async function movieLogoCartaz(movies, type = "tv"){
    try{   
        //url logos/cartaz do filme
        const urlImages = `https://api.themoviedb.org/3/tv/${movies}/images?include_image_language=pt&language=pt-BR`    
        const response = await fetch(urlImages, optionsGET);
...
async function iniciandoAPIMoviesSemelhante(postId, type = "tv"){
    try{
        //url filmes populares   
        const url = `https://api.themoviedb.org/3/tv/${postId}/similar?language=en-US&page=1`;    
        const response = await fetch(url, optionsGET);
...
async function iniciandoAPImovieVideos(movies, language, type = "tv"){
    try{   
        //url logos/cartaz do filme
        const urlImages = `https://api.themoviedb.org/3/tv/${movies}/videos?language=${language}`    
        const response = await fetch(urlImages, optionsGET);
...
async function documentTitle(postId){
    ...
    titles[0].innerHTML = `${detalhe.name} - CineGo!`;
}
...
function containerDescricao(movies, movieVideos){
    ...
    durationTitle.innerHTML = `${movies.number_of_seasons} Temporada(s)`;
...
function containerLogo(imageLogo, detalhe){
    ...
    if(imageLogo.logos.length > 0){  
        ...
        nameTitle.innerHTML = `${detalhe.name}`;
    }
}
